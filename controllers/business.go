...
//列表页
func (self *BusinessController) List() {
	self.Data["pageTitle"] = "商务合作"
	self.display()
	// self.TplName = "aboutus/companyprofile/list.html"
}
//新增页
func (self *BusinessController) Add() {
	self.Data["pageTitle"] = "商务合作新增"
	self.display()
}
//添加修改接口
func (self *BusinessController) AjaxSave() {
	...
	if ArticleId == 0 {
		...
		// self.ajaxMsg("", MSG_OK)
		self.redirect(beego.URLFor("BusinessController.List"))
	}
...
//列表接口
func (self *BusinessController) Table() {
...
//查看详情 
func (self *BusinessController) Detail() {
	// self.Data["ApiCss"] = true
	self.Data["pageTitle"] = "商务合作详情"
...
//编辑
func (self *BusinessController) Edit() {
	self.Data["pageTitle"] = "商务合作编辑"
	id, _ := self.GetInt("id", 0)
	detail, _ := models.JoinUsGetById(id)
	row := make(map[string]interface{})
	row["id"] = detail.Id
...
//删除 接口
func (self *BusinessController) AjaxDel() {	
	id, _ := self.GetInt("id")
