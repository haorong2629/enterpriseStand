...
type JoinTeamController struct {
	BaseController
}
//列表页
func (self *JoinTeamController) List() {
	self.Data["pageTitle"] = "加入我们"
	self.display()
	// self.TplName = "aboutus/companyprofile/list.html"
}
//新增页
func (self *JoinTeamController) Add() {
	self.Data["pageTitle"] = "加入我们新增"
	self.display()
}
//添加修改接口
func (self *JoinTeamController) AjaxSave() {
	...
	if ArticleId == 0 {
		...
		// self.ajaxMsg("", MSG_OK)
		self.redirect(beego.URLFor("JoinTeamController.List"))
	}
...
//列表接口
func (self *JoinTeamController) Table() {
...
//查看详情 
func (self *JoinTeamController) Detail() {
	// self.Data["ApiCss"] = true
	self.Data["pageTitle"] = "加入我们详情"
...
//编辑
func (self *JoinTeamController) Edit() {
	self.Data["pageTitle"] = "加入我们编辑"
	id, _ := self.GetInt("id", 0)
	detail, _ := models.JoinUsGetById(id)
	row := make(map[string]interface{})
	row["id"] = detail.Id
...
//删除 接口
func (self *JoinTeamController) AjaxDel() {	
	id, _ := self.GetInt("id")
