...
type CompanyConceptController struct {
	BaseController
}
//列表页
func (self *CompanyConceptController) List() {
	self.Data["pageTitle"] = "公司理念"
	self.display()
	// self.TplName = "aboutus/companyprofile/list.html"
}
//新增页
func (self *CompanyConceptController) Add() {
	self.Data["pageTitle"] = "公司理念新增"
	self.display()
}
//添加修改接口
func (self *CompanyConceptController) AjaxSave() {
	...
	if ArticleId == 0 {
		...
		// self.ajaxMsg("", MSG_OK)
		self.redirect(beego.URLFor("CompanyConceptController.List"))
	}
...
//列表接口
func (self *CompanyConceptController) Table() {
...
//查看详情 
func (self *CompanyConceptController) Detail() {
	// self.Data["ApiCss"] = true
	self.Data["pageTitle"] = "公司理念详情"
...
//编辑
func (self *CompanyConceptController) Edit() {
	self.Data["pageTitle"] = "公司理念编辑"
	id, _ := self.GetInt("id", 0)
	detail, _ := models.AboutUsGetById(id)
	row := make(map[string]interface{})
	row["id"] = detail.Id
...
//删除 接口
func (self *CompanyConceptController) AjaxDel() {	
	id, _ := self.GetInt("id")
