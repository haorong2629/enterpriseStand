...
type CompanyServiceController struct {
	BaseController
}
//列表页
func (self *CompanyServiceController) List() {
	self.Data["pageTitle"] = "服务客户"
	self.display()
	// self.TplName = "aboutus/companyprofile/list.html"
}
//新增页
func (self *CompanyServiceController) Add() {
	self.Data["pageTitle"] = "服务客户新增"
	self.display()
}
//添加修改接口
func (self *CompanyServiceController) AjaxSave() {
	...
	if ArticleId == 0 {
		...
		// self.ajaxMsg("", MSG_OK)
		self.redirect(beego.URLFor("CompanyServiceController.List"))
	}
...
//列表接口
func (self *CompanyServiceController) Table() {
...
//查看详情 
func (self *CompanyServiceController) Detail() {
	// self.Data["ApiCss"] = true
	self.Data["pageTitle"] = "服务客户详情"
...
//编辑
func (self *CompanyServiceController) Edit() {
	self.Data["pageTitle"] = "服务客户编辑"
	id, _ := self.GetInt("id", 0)
	detail, _ := models.AboutUsGetById(id)
	row := make(map[string]interface{})
	row["id"] = detail.Id
...
//删除 接口
func (self *CompanyServiceController) AjaxDel() {	
	id, _ := self.GetInt("id")
